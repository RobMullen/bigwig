...
handle(Req0, State) ->
    ...
    %% io:format("bigwig_http_etop2:handle/2 Path is:~p, Method is:~p~nReq0:~p~n",[Path,Method,Req0]),
...
handle_path(Method, Path, Req, State) ->
    %%io:format("~p:handle/2 not matched, Path is:~p, Method is:~p~n",[?MODULE,Path,Method]),
    not_found(Req, State).
